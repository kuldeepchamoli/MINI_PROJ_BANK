...
int manager_activate_deactivate_customer(int *client_socket) {
    ...
    int result = -1, index = -1, flag = 0;
    
    

    struct Customer_S temporary;
    while (read(fd, &temporary, sizeof(struct Customer_S)) > 0) {
        index++;
        if (strcmp(temporary.username, customer_username) == 0) {
            flag = 1;   // match found
            break;
        }
    }
    ...
    if(flag == 1) {
        ...
        lock.l_start = index * sizeof(struct Customer_S);
        lock.l_len = sizeof(struct Customer_S);

        fcntl(fd, F_SETLKW, &lock);


        lseek(fd, index * sizeof(struct Customer_S), SEEK_SET);
        read(fd, &temporary, sizeof(struct Customer_S));

        lseek(fd, -1 * sizeof(struct Customer_S), SEEK_CUR);

        temporary.active = do_activate == 0 ? 0 : 1;
        write(fd, &temporary, sizeof(struct Customer_S));
